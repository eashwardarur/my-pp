...
// Removed static import of SplitText to avoid module not found when bonus plugin isn't installed
  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        // Always use manual split to avoid dependency on bonus plugin
        ...
        chars.forEach(ch => {
          ...
          const w = rect.width || ch.offsetWidth || 0;
          if (!w) return;
          ...
          // compute travel based on direction
          const steps = rolls + 1;
          let startX = 0;
          let finalX = -steps * w; // slide left to reveal
